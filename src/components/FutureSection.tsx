...
const FutureSection: React.FC = () => {
  const [plans, setPlans] = useState<FuturePlan[]>([
    {
      ...
      description: 'Planear unas vacaciones juntos en la costa'
    },
    {
      id: 'plan-2',
      title: 'Aprender a cocinar',
      description: 'Tomar clases de cocina juntos para preparar comidas especiales'
    }
  ]);
  ...
  const handleSecretButtonMouseDown = () => {
    const timer = setTimeout(() => {
      ...
      const audio = new Audio('https://p.scdn.co/mp3-preview/e2f5edb569c73916235b47baf61cf16c4b901e28');
      audio.play();
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/80 dark:bg-blue-900/30 rounded-xl p-6 shadow-xl mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-blue-800 dark:text-blue-300">Nuestros planes futuros</h3>
              <p className="text-blue-700 dark:text-blue-400">Melodías que aún no hemos compuesto</p>
            </div>
          </div>
          ...
          <ul className="space-y-4 mb-8">
            {plans.map((plan) => (
              ...
              >
                <h4 className="text-lg font-semibold text-blue-800 dark:text-blue-300">{plan.title}</h4>
                <p className="text-blue-700 dark:text-blue-400">{plan.description}</p>
              </li>
            ))}
          </ul>
