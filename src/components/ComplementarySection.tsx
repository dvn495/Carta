...
const ComplementarySection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const element1Particles: { x: number, y: number, angle: number, speed: number, size: number }[] = [];
    const element2Particles: { x: number, y: number, angle: number, speed: number, size: number }[] = [];
    ...
    const animate = () => {
      ...
      ctx.bezierCurveTo(controlX1, controlY1, controlX2, controlY2, centerX + 100, centerY);
      
      const gradient2 = ctx.createLinearGradient(centerX - 100, centerY, centerX + 100, centerY);
      ...
      element1Particles.forEach(particle => {
        particle.angle += 0.02;
        particle.x = centerX - 100 + Math.cos(particle.angle) * (30 + Math.sin(time + particle.angle) * 10);
        particle.y = centerY + Math.sin(particle.angle) * (30 + Math.cos(time + particle.angle) * 10);
      ...
      ctx.arc(centerX, centerY - 30 + Math.sin(time) * 10, pulseSize, 0, Math.PI * 2);
  ...
  return (
    <section className="py-20 px-4 bg-white dark:bg-blue-950 transition-colors duration-500" ref={containerRef}>
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-center text-blue-700 dark:text-blue-400 mb-8 max-w-2xl mx-auto">
          "Siento que tú me complementas y viceversa, ciertas cosas que tú tienes que me hacen falta"
        </p>
        ...
        <div className="bg-blue-100/70 dark:bg-blue-900/30 p-6 rounded-xl">
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-1 p-4 rounded-lg bg-white/70 dark:bg-blue-800/30 shadow-md">
              <h3 className="text-xl font-bold mb-2 text-blue-700 dark:text-blue-300">Fuerza</h3>
              <p className="text-blue-800 dark:text-blue-400">
                Tu determinación y energía iluminan mi camino. Me inspiras a ser mejor cada día con tu dedicación y pasión.
              </p>
            </div>
            
            <div className="flex-1 p-4 rounded-lg bg-white/70 dark:bg-blue-800/30 shadow-md">
              <h3 className="text-xl font-bold mb-2 text-pink-700 dark:text-pink-300">Gracia</h3>
              <p className="text-pink-800 dark:text-pink-400">
                Tu delicadeza y elegancia traen belleza a mi vida. Me enseñas a ver el mundo con nuevos ojos llenos de admiración.
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="italic text-blue-900 dark:text-blue-200">
              "Creo que nosotros somos la prueba de que los opuestos se atraen"
            </p>
          </div>
        </div>
      </div>
